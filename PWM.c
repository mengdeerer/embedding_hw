...
void PWMStart(uint32_t ui32Freq_Hz)
{
    ...
    PWMGenDisable(PWM0_BASE, PWM_GEN_3); // 使能PWM0模块的2号发生器(因为4号PWM是2号发生器产生的)

    PWMGenPeriodSet(PWM0_BASE, PWM_GEN_3, g_ui32SysClock / ui32Freq_Hz);                 // 根据Freq_Hz设置PWM周期
    PWMPulseWidthSet(PWM0_BASE, PWM_OUT_7, (PWMGenPeriodGet(PWM0_BASE, PWM_GEN_3) / 2)); // 设置占空比为50%

    PWMGenEnable(PWM0_BASE, PWM_GEN_3); // 使能PWM0模块的2号发生器(因为4号PWM是2号发生器产生的)
}
...
void PWMStop()
{
    PWMGenDisable(PWM0_BASE, PWM_GEN_3); // M0PWM4(PG0)停止产生PWM信号
}
